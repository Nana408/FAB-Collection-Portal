...
$(document).ready(function () {
    ...
    $('#payment-verify').click(function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.Status == 1) {
                    $("#cc-totalAmount").val(data.TotalAmount);
                    $("#cc-status").val(data.PaidStatus);
    ...
    $("#cc-amount").on("keyup", function () {
        ...
        if (!valid) {
            swal("Provide a valid amount");
            this.value = val.substring(0, val.length - 1);
        }
    });

    $('#payment-pay').prop('disabled', true);

    $('#payment-pay').click(function (e) {
        e.preventDefault();
        e.stopPropagation();

        $('#payment-pay').prop('disabled', true);

        var invoiceNumber = $("#cc-invoice").val();
        var currency = $("#cc-currency").val();
        var bankBanchSortCode = $("#cc-bankBanchSortCode").val();
        var accountNumber = $("#cc-accountNumber").val();
        ...
        if (bankBanchSortCode == '' && $('#select option:selected').text() == 2) {
            swal("Validation Error", "Provide bank branch sort code", "error");
            $('#payment-pay').prop('disabled', false);
            return false;
        }
        if (accountNumber == '' && $('#select option:selected').text() == 2) {
            swal("Validation Error", "Provide account number", "error");
            $('#payment-pay').prop('disabled', false);
            return false;
        }
        if (chequeNumber == '' && $('#select option:selected').text() == 2) {
            swal("Validation Error", "Provide cheque number", "error");
            $('#payment-pay').prop('disabled', false);
            return false;
        }
        if (valueDate == '' && $('#select option:selected').text() == 2) {
            swal("Validation Error", "Provide value date", "error");
            $('#payment-pay').prop('disabled', false);
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.Status == 1) {
                    ...
                    swal("Failed Payment", data.Message, "error")
                    $('#payment-pay').prop('disabled', false);
                }
